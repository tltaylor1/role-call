...
in advance. Subphases 12 of 12 are built and merged: the foundation, the
operators and audit spine, both ingestion surfaces, the derivation
engine, the credential and privilege findings, the synthetic sample
account that exercises all of them, the page, the governance records
that put a typed owner, a purpose, flags, and attestations on
identities and groups, the review campaigns with their evidence
export, the risk report with the escaped exports, and the proof
subphase: the coverage floor, the mutation check, the container
lint and base image scan, the write budget, the runbook, and the
route surface asserted against the documents. Phase 0's design
